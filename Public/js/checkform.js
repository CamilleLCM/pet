...
function isMobile(mobile){
	if(mobile!=""){
		//var reg=/^(1[3-9]{1}[0-9]|15[0|3|6|7|8|9]|18[8|9])\d{8}$/;	
...
且结束如期>=起始日期
输入：startDate：起始日期，字符串级 endDate：结束如期，字符串
返回：如果通过验证返回true,否则返回false
*/
...
function checkSelect( checkboxID ) {
	...
	if( document.all(checkboxID).length > 0 ) {
		...
		if( document.all(checkboxID).checked )check = 1;
	}
	return check;
}
