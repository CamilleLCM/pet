...
var $ = jQuery, w = $(window), ready = {
	getPath: function(){
		...
		$('head').prepend('<link rel="stylesheet" id="layerCss" type="text/css" href="'+ pathUrl +'skin/layer.css"  />' + ie6PNG);
	}, global: {
...
//layer默认内置方法，其它形式都可通过$.layer()任意拓展。
window['layer'] = {
	v : '1.5.0',	//版本号
...
//层容器
Class.prototype.space = function(){
	var times = this.index;	
	var config = this.config, dialog = config.dialog, frame = [
		...
		'<div class="xubox_tips xubox_tips_' + config.skin +'"><div class="xubox_tipsMsg">'+ config.tips.msg +'</div><i class="xulayer_png32"></i></div>'
	];		
	
	var shade = '' , border = '';
	this.zIndex = config.zIndex + times;
	
	var zIndex = this.zIndex, shadeStyle = "z-index:"+ zIndex +"; background:"+ config.shade[1] +"; opacity:"+ config.shade[0] +"; filter:'alpha(opacity="+ config.shade[0]*100 +")'; filter:alpha(opacity="+ config.shade[0]*100 +");";
...
//插入layer
Class.prototype.adjust = function(){
	...
	var page = config.page, setSpace = function(){
		space = othis.space();
		$("body").append(space[0]);
	};
	switch(config.type){
...
//调整layer	
Class.prototype.pagetype = function(times){
	...
	switch(config.type){
		...
		break;
		case 4 :
			var top = $(config.tips.follow).offset().top - $(config.tips.follow).outerHeight() - layerE.outerHeight() + 15;
			var left = $(config.tips.follow).offset().left + layerE.outerWidth()/2;
...
//自适应宽高
Class.prototype.autoArea = function(times){
	...
	(config.offset[1] == '50%' || config.offset[1] == '') ? layerE.css({marginLeft : -layerE.outerWidth()/2}) : layerE.css({marginLeft : 0});
...
Class.prototype.openLayer = function(){
	...
	layer.shift = function(type, rate){
		...
		switch(type){
			case 'left-top':
				layerE.css({left : cutWth + config.border[0], top : -layerE.outerHeight()}).animate({top : config.border[0]}, rate, function(){
					iE6 && othis.IE6();	
				});
			break; 
			case 'right-top':
				layerE.css({left : ww - cutWth - config.border[0], top : -layerE.outerHeight()}).animate({top : config.border[0]}, rate, function(){
					iE6 && othis.IE6();	
				});
			break; 
			case 'left-bottom':
				layerE.css({left : cutWth + config.border[0], top : wh}).animate({top : wh - layerE.outerHeight() - config.border[0]}, rate, function(){
					iE6 && othis.IE6();	
				});
			break; 
			case 'right-bottom':
				layerE.css({left : ww - cutWth - config.border[0], top : wh}).animate({top : wh - layerE.outerHeight() - config.border[0]},rate,function(){
					iE6 && othis.IE6();	
				});
			break;
		};	
