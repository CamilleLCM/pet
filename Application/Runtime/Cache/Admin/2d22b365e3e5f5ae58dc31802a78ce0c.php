...
<!-- left menu ends -->
        <div class="span9">
            <h1 class="page-title">修改知道和趣事</h1>
...
<div class="well">
    ...
    <div id="myTabContent" class="tab-content">
      <div class="tab-pane active in" id="home">
          <form id="subform" enctype="multipart/form-data" action="<?php echo U('Admin/Message/edit_do',array('id'=>$result['id']));?>" method="post">
              <label>标题：</label>
              <input type="text" value="<?php echo ($result['title']); ?>" name='title'/>
              <label>图片：</label>
              </br>
              <img alt="" width="80" src="Public/Uploads/<?php echo ($result["img"]); ?>"></br></br>
              <input type="file" name="img" id="img" >
              <label>分类：</label>
              <select name="category_id" id="category_id">
                  <?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($result['category_id']==$vo['id']){ ?>
                      selected="selected"  <?php } ?> ><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>

              </select>
              <label>内容简介：</label>
              <textarea style="margin: 0px 0px 10px; width: 653px; height: 107px;" name="content"><?php echo ($result['content']); ?></textarea>

              <label></label>
              <button class="btn btn-primary" onclick="document.getElementById('subform').submit();return false">
                  <i class="icon-save"></i> 编辑</button>
          </form>
      </div>
  </div>
