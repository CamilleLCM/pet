...
<div class="well">
    ...
    <form id="subform" action="index.php?m=Admin&c=User&a=edit_do&uid=<?php echo ($user['uid']); ?>" enctype="multipart/form-data" method="post">
        <label>用户名：</label>
        <input type="text" value="<?php echo ($user['name']); ?>" readonly="readonly"/>
        <label>图片：</label>
        <img alt="" width="80" src="Public/Uploads/<?php echo ($user["img"]); ?>">
        <input type="file" name="img" id="img" >
        <label>用户真实姓名：</label>
         <input type="text" value="<?php echo ($user['realname']); ?>" name='realname' />
        <label>手机号：</label>
        <input type="text" value="<?php echo ($user['mobile']); ?>" name='mobile' />
        <label>邮箱：</label>
       ...
       <select name="type" id="type">
               ...
               <option value="b" <?php if($user['type']=='b'){ ?> selected="selected"  <?php } ?> >普通会员</option>

       </select> 
        <label>用户状态</label>
        <select name="status" id="status">                                                         
              <option value="0" >正常</option>
              <option value="1" <?php if($user['status']=='1'){ ?> selected="selected"  <?php } ?>>锁定</option>
              <option value="3" <?php if($user['status']=='3'){ ?> selected="selected"  <?php } ?>>删除</option>
      </select>
       <label></label>
      <button class="btn btn-primary" onclick="document.getElementById('subform').submit();return false"><i class="icon-save"></i> 修改</button>
    </form>
      </div>
  </div>
