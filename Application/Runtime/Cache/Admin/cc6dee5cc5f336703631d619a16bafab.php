...
<div class="btn-toolbar">
    <button class="btn btn-primary" onclick="window.location.href='index.php?m=Admin&c=User&a=add'"><i class="icon-plus"></i> 
	添加用户 </button>
  <div class="btn-group">
  </div>
</div>
<div class="well">
    <table class="table">
      ...
      <tbody>
        <?php if(is_array($result)): foreach($result as $key=>$user): ?><tr>
				<tr>
				<td class="center"><?php echo ($user['name']); ?></td>
                <td class="center">
                    <img alt="" width="80" src="Public/Uploads/<?php echo ($user["img"]); ?>">
                </td>
                <td class="center"><?php echo ($user['realname']); ?></td>
                <td class="center"><?php echo ($user['mobile']); ?></td>
				<td class="center"><?php echo (date("Y-m-d H:i:s",$user['ctime'])); ?></td>
				<td>
				...
				<td>
		              <a href="index.php?m=Admin&c=User&a=edit&uid=<?php echo ($user['uid']); ?>"><i class="icon-pencil"></i></a>
		              <a onClick="return confirm('确认要删除？')" href="index.php?m=Admin&c=User&a=del_by_uid&uid=<?php echo ($user['uid']); ?>" role="button" data-toggle="modal"><i class="icon-remove"></i></a>
	            </td>
